...
function doPost(e){
 ...
 // var intake = json.getOwnPropertyNames(json);
  ...
  var order = {   
   ...
   "Company" : json.shipping.company,
   "Street Address" : json.billing.address_1,
   "Suite" : json.billing.address_2,
   "City" : json.billing.city,
   "State" : json.billing.state,
   "Postal_Code" : json.billing.postcode,
   "Country" : json.billing.country,
   "S Company" : json.shipping.company,
   "S First Name" : json.shipping.first_name,
   "S Last Name" : json.shipping.last_name,
   "S Street Address" : json.shipping.address_1,
   "S Suite" : json.shipping.address_2,
   "S City" : json.shipping.city,
   "S State" : json.shipping.state,
   "S Postal_Code" : json.shipping.postcode,
   "S Country" : json.shipping.country,
   "Paid By" : json.payment_method_title,
   "Shipping" : json.shipping_total,
   "Total" : json.total,
   "TotalTax" : json.total_tax,
   "Invoice" : json.meta_data[5]._wcpdf_invoice_number,
  };
  console.log(order);
  for(i=0;i<json.line_items.length; i++){
    var sValue = json.line_items[i].sku;
    var supplier = splice(indexof(json.line_items[i].sku, 2));
    var qValue = json.line_items[i].quantity;
